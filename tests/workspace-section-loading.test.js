...
function testStorageAndActionContracts() {
  ...
  check(wordEditing.includes('isStandaloneTemporaryAction') && wordEditing.includes('!state.root?.contains(trigger) && !isStandaloneTemporaryAction'), 'standalone Temporary candidate confirm and dismiss actions should remain valid outside the Advanced Settings root');
  ...
  check(customSelect.includes('const isInTemporaryCandidate') && customSelect.includes('const relativeLeft = viewportLeft - shellRect.left;') && customSelect.includes('The standalone Temporary Names panel can be transformed or dragged'), 'Temporary candidate category menus should stay trigger-relative inside a transformed or dragged standalone panel');
  ...
  check(advancedWordEditingCss.includes('max-height: min(292px, 55vh)') && advancedWordEditingCss.includes('grid-auto-rows: 52px') && advancedWordEditingCss.includes('height: 52px'), 'Temporary candidate lists should use fixed-height boxes and show no more than five complete rows by default');
  check(advancedWordEditingCss.includes('.awe-temporary-candidate-row:has(.lm-custom-select.is-open)') && advancedWordEditingCss.includes(':not(:has(> .awe-temporary-candidate-row:nth-child(6)))') && advancedWordEditingCss.includes('overflow: visible'), 'an open Temporary category menu should float beyond its naming box boundary without exposing hidden rows from larger lists');
  check(wordEditing.includes('fitStandaloneTemporaryPanelToCandidates') && wordEditing.includes('standaloneTemporaryPanelDefaultHeight') && wordEditing.includes('visibleRows = Math.min(5, rows.length)'), 'standalone Temporary Names height should auto-fit one-to-five entries and use five fixed rows as its larger-list default');
  check(wordEditing.includes('temporaryPanelBaseMinimumHeight') && wordEditing.includes('panel?.querySelector(\'.awe-temporary-candidate-row\') ? 0 : 150') && advancedWordEditingCss.includes('.awe-temporary-candidates-panel:has(.awe-temporary-candidate-row)'), 'non-empty Temporary Names panels should drop the empty-state minimum height and fit short lists without blank space');
  check(wordEditing.includes('function reconcileStandaloneTemporaryPanelHeight') && wordEditing.includes('if (itemCount <= 5)') && wordEditing.includes('delete panel.dataset.aweUserSized') && wordEditing.includes('reconcileStandaloneTemporaryPanelHeight(panel);'), 'Temporary Names should force exact short-list auto-fit after every render even when the user previously adjusted its height');
  check(wordEditing.includes('delete panel.dataset.aweUserSized') && !wordEditing.includes('height: panel.dataset.aweUserSized'), 'Temporary Names should forget manual height when closed and recalculate it from the next item count');
  check(wordEditing.includes('minimumHeight: standaloneTemporaryPanelDefaultHeight(panel)') && wordEditing.includes('Math.max(minimumHeight, Math.min(maxHeight, requestedHeight))'), 'a scrollable Temporary Names panel should not resize below its five-row default height');
  ...
  check(wordEditing.includes('function syncTemporaryPanelResizeAvailability') && wordEditing.includes('hasMoreThanDefaultRows') && wordEditing.includes('hasMoreThanDefaultRows || hasOverflow') && wordEditing.includes("handle.setAttribute('aria-disabled', String(!canResize))"), 'Temporary Names height resizing should remain available for lists longer than five rows even after expansion removes the scrollbar');
  check(advancedWordEditingCss.includes('.can-resize-height .awe-temporary-height-resize-handle') && advancedWordEditingCss.includes('pointer-events: none') && advancedWordEditingCss.includes('pointer-events: auto'), 'Temporary Names resize grip should stay interactive whenever its count or overflow permits height adjustment');
